...
//	}
	...
	public void updateCommentOnVote(String posterName, boolean isUpvote, String commID) throws SQLException {
        ...
        int updatedComment;
        try{
            updatedComment = stmnt.executeUpdate("UPDATE UserComment SET voteNum = voteNum + " + vote + " WHERE CommID = " + commID + "");
            assert updatedComment == 0;
            updateRepComment(commID);
        } catch (SQLException e) {
            stmnt.executeUpdate("DELETE FROM CommentVote WHERE PosterName = '" +posterName+ "' AND CommID = '" + commID + "'");
            throw new SQLException();
        }
	}
